...
function addCase() {
	...
	$case = json_decode($request->getBody());
	$sql = "INSERT INTO employee (firstName, lastName) VALUES ( :firstName, :lastName)";
	try {
		$db = getConnection();
		$stmt = $db->prepare($sql);
		$stmt->bindParam("grapes", $wine->fisrtName);
		$stmt->bindParam("country", $wine->lastName);
		$stmt->execute();
		$case->id = $db->lastInsertId();
		$db = null;
